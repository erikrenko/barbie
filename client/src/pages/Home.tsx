...
function SummaryPage({ go }: { go: (n: number) => void }) {
  return <div className="summary-page page-grid"><div className="summary-intro"><SectionTag tone="yellow">19 · ZAKLJUČEK</SectionTag><h2>OD IDEJE DO <em>DANAŠNJE BARBIE</em></h2><p>Barbiejina zgodba ni ena sama slika ali ena sama igrača. To je zaporedje idej, ljudi in možnosti, ki so se skozi leta širile.</p><div className="summary-statement"><span>GLAVNA MISEL</span><strong>IGRA LAHKO ODPRE VRATA V NOVO ZGODBO.</strong><small>Najprej nekdo opazi, vpraša in poskusi. Nato lahko drugi dodajo svojo domišljijo.</small></div><div className="page-actions"><BackButton onClick={() => go(17)} /><NextButton onClick={() => go(19)}>USTVARI SVOJO KARTICO</NextButton></div></div><div className="summary-panel"><div className="panel-heading"><span>BARBIJINA POT SKOZI ČAS</span><b>1959 → DANES</b></div><div className="timeline-list">{summarySteps.map(([number, title, text]) => <div className="timeline-step" key={number}><span className="timeline-number">{number}</span><div><strong>{title}</strong><p>{text}</p></div></div>)}</div><div className="summary-takeaway"><Star size={18} /><span>TI LAHKO IZBEREŠ, KAJ BO TVOJA JUNAKINJA ODKRIVALA, USTVARJALA IN POSTALA.</span></div></div></div>;
}
